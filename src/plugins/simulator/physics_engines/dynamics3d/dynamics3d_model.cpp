...
namespace argos {
   ...
   void CDynamics3DModel::Reset() {
      ...
      btTransform cModelResetTransform(
         ARGoSToBullet(GetEmbodiedEntity().GetOriginAnchor().Orientation),
   ...
   void CDynamics3DModel::CalculateBoundingBox() {
      ...
      for(CDynamics3DBody::TVector::iterator itBody = m_vecLocalBodies.begin();
         ...
         if(bAabbVectorInitRequired == true) {
            ...
            cAabbMin.setX(cAabbMin.getX() < cBodyAabbMin.getX() ? 
                          cAabbMin.getX() : cBodyAabbMin.getX());
            cAabbMin.setY(cAabbMin.getY() < cBodyAabbMin.getY() ? 
                          cAabbMin.getY() : cBodyAabbMin.getY());
            cAabbMin.setZ(cAabbMin.getZ() < cBodyAabbMin.getZ() ? 
                          cAabbMin.getZ() : cBodyAabbMin.getZ());
            cAabbMax.setX(cAabbMax.getX() > cBodyAabbMax.getX() ? 
                          cAabbMax.getX() : cBodyAabbMax.getX());
            cAabbMax.setY(cAabbMax.getY() > cBodyAabbMax.getY() ? 
                          cAabbMax.getY() : cBodyAabbMax.getY());
            cAabbMax.setZ(cAabbMax.getZ() > cBodyAabbMax.getZ() ? 
                          cAabbMax.getZ() : cBodyAabbMax.getZ());
         }
      }
